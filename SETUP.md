...
3. Install **PyTorch** - the one required dependency (CPU is plenty for this project):
...
## Optional: the companion notebook

`explore.ipynb` shows what you built thinking - the causal averaging matrix, a real
attention map on real Shakespeare, bigram-vs-GPT loss curves, and the same prompt
sampled as training progresses. It's best after the milestones are done and needs two
extras:

```bash
pip install jupyter matplotlib
jupyter notebook explore.ipynb   # or open it in VS Code / Cursor directly
```

Skip it entirely and the milestones still work - it's dessert, not dinner.
...
```text
Read AGENTS.md and tutor me through this project. Do not write data.py, attention.py, model.py, or train.py for me.
```
...
milestone tracking; the learner writes `data.py`, `attention.py`, `model.py`, and
`train.py`.
